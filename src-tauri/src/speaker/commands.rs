// Salesly AI Speech Detection, and capture system audio (speaker output) as a stream of f32 samples.
...
async fn run_vad_capture(
    ...
    while let Some(sample) = stream.next().await {
        ...
        while buffer.len() >= config.hop_size {
            ...
            // DEBUG: print every 50 chunks (~0.5s) so terminal isn't flooded
            if (speech_chunks + silence_chunks) % 50 == 0 {
                println!("[VAD] rms={:.4} peak={:.4} is_speech={} in_speech={} speech_chunks={} silence_chunks={}", rms, peak, is_speech, in_speech, speech_chunks, silence_chunks);
            }

            if is_speech {
                if !in_speech {
                    // Speech START detected
                    println!("[VAD] Speech START detected (rms={:.4} peak={:.4})", rms, peak);
                    in_speech = true;
                    speech_chunks = 0;
                ...
                if in_speech {
                    ...
                    if silence_chunks >= config.silence_chunks {
                        ...
                        if speech_chunks >= config.min_speech_chunks && !speech_buffer.is_empty() {
                            ...
                            let normalized_buffer = normalize_audio_level(&speech_buffer, 0.1);
                            if let Ok(b64) = samples_to_wav_b64(sr, &normalized_buffer) {
                                println!("[VAD] Speech EMITTED — {} samples ({:.2}s)", normalized_buffer.len(), normalized_buffer.len() as f32 / sr as f32);
                                let _ = app.emit("speech-detected", b64);
                            } else {
                            ...
                            println!("[VAD] Speech DISCARDED — speech_chunks={} < min={}", speech_chunks, config.min_speech_chunks);
                            let _ = app.emit(
                                "speech-discarded",
