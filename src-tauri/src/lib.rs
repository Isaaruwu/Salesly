...
pub fn run() {
    ...
    let mut builder = tauri::Builder::default()
        .plugin(
            tauri_plugin_sql::Builder::default()
                .add_migrations("sqlite:salesly.db", db::migrations())
                .build(),
        )
    ...
    let mut builder = builder
        .invoke_handler(tauri::generate_handler![
            ...
            window::open_clients,
            window::toggle_clients,
            window::open_pre_meeting,
            window::close_pre_meeting,
            window::open_recording,
            window::close_recording,
            window::open_post_meeting_summary,
            window::close_post_meeting_summary,
            window::move_window,
        ...
        .setup(|app| {
            ...
            if app_handle.get_webview_window("clients").is_none() {
                if let Err(e) = window::create_clients_window(&app_handle) {
                    eprintln!("Failed to pre-create clients window on startup: {}", e);
                }
            }
            if app_handle.get_webview_window("pre_meeting").is_none() {
                if let Err(e) = window::create_pre_meeting_window(&app_handle) {
                    eprintln!("Failed to pre-create pre-meeting window on startup: {}", e);
                }
            }
            if app_handle.get_webview_window("recording").is_none() {
                if let Err(e) = window::create_recording_window(&app_handle) {
                    eprintln!("Failed to pre-create recording window on startup: {}", e);
                }
            }
            if app_handle.get_webview_window("post_meeting_summary").is_none() {
                if let Err(e) = window::create_post_meeting_summary_window(&app_handle) {
                    eprintln!("Failed to pre-create post-meeting summary window on startup: {}", e);
                }
            }
