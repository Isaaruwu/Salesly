...
import { getPlatform, safeLocalStorage, trackAppStart, BASE_ADVISOR_PROMPT } from "@/lib";
import { upsertAdvisorSystemPrompt, getAdvisorSystemPrompt } from "@/lib/database/system-prompt.action";
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      // Seed advisor prompt into DB and use it as default if nothing is saved
      try {
        await upsertAdvisorSystemPrompt(BASE_ADVISOR_PROMPT);
        const savedPrompt = safeLocalStorage.getItem(STORAGE_KEYS.SYSTEM_PROMPT);
        if (!savedPrompt || savedPrompt === DEFAULT_SYSTEM_PROMPT) {
          const advisorPrompt = await getAdvisorSystemPrompt();
          if (advisorPrompt) {
            setSystemPrompt(advisorPrompt);
            safeLocalStorage.setItem(STORAGE_KEYS.SYSTEM_PROMPT, advisorPrompt);
          }
        }
      } catch (error) {
        console.debug("Failed to seed advisor system prompt:", error);
      }
      ...
      try {
        await Promise.all([
          ...
          invoke("set_content_protected", {
            enabled: customizable.contentProtection?.isEnabled ?? true,
          }),
        ]);
      } catch (error) {
