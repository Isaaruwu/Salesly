...
export function useHistory(): UseHistoryReturn {
  ...
  // Refresh when a conversation is saved from any window
  useEffect(() => {
    ...
    // Same-window signal (overlay and dashboard share the same window)
    window.addEventListener("conversationSaved", scheduleRefresh);

    // Cross-window signal via localStorage storage event
    const handleStorage = (e: StorageEvent) => {
      if (e.key === "salesly_last_conversation_saved") scheduleRefresh();
    };
    window.addEventListener("storage", handleStorage);

    return () => {
      if (debounceRef.timer) clearTimeout(debounceRef.timer);
      window.removeEventListener("conversationSaved", scheduleRefresh);
