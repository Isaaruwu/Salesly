...
import { resampleAudioTo16k } from "@/lib/utils";
import {
      ...
      try {
        speechUnlisten = await listen("speech-detected", async (event) => {
          try {
            ...
            // Resample to 16 kHz mono — system audio is captured at the
            // device's native rate (typically 48 kHz on macOS) which causes
            // Whisper and other STT providers to return empty transcriptions.
            const audioBlob = await resampleAudioTo16k(rawBlob);
