...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    // Send request
    // Debug: measure audio RMS to confirm it has signal
    const debugBuffer = await audio.arrayBuffer();
    const debugView = new DataView(debugBuffer);
    let sumSq = 0, debugSamples = 0;
    for (let i = 44; i < debugBuffer.byteLength - 1; i += 2) {
      const s = debugView.getInt16(i, true) / 32768.0;
      sumSq += s * s;
      debugSamples++;
    }
    const rms = debugSamples > 0 ? Math.sqrt(sumSq / debugSamples) : 0;
    console.log("[STT] Audio RMS:", rms.toFixed(4), "| Duration:", (debugSamples / 48000).toFixed(2) + "s", "| Size:", audio.size);

    console.log("[STT] Sending audio to STT provider", {
    ...
    console.log("[STT] Configured responseContentPath:", rawPath, "→ normalized path:", path);
    const transcription = (getByPath(data, path) || "").trim();
    console.log("[STT] Extracted transcription:", transcription);

    if (!transcription) {
      throw new Error("No transcription found in response. Check your provider's Response Content Path setting.");
    }
