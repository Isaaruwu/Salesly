...
export const getCustomizableState = (): CustomizableState => {
  try {
    ...
    return {
      ...
      contentProtection:
        parsedState.contentProtection ?? DEFAULT_CUSTOMIZABLE_STATE.contentProtection,
    };
  } catch (error) {
...
/**
 * Update content protection state
 */
export const updateContentProtection = (isEnabled: boolean): CustomizableState => {
  const currentState = getCustomizableState();
