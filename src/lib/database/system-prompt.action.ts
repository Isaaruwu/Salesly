...
export const ADVISOR_PROMPT_NAME = "Advisor Co-Pilot";
...
/**
 * Ensures the Advisor Co-Pilot prompt exists in the DB.
 * Inserts it if missing; updates the text if it already exists (keeps it in sync with code).
 */
export async function upsertAdvisorSystemPrompt(promptText: string): Promise<void> {
  const db = await getDatabase();
  const existing = await db.select<SystemPrompt[]>(
  ...
  if (existing.length === 0) {
    await db.execute(
      "INSERT INTO system_prompts (name, prompt) VALUES (?, ?)",
      [ADVISOR_PROMPT_NAME, promptText]
    );
  } else {
    await db.execute(
      "UPDATE system_prompts SET prompt = ? WHERE name = ?",
      [promptText, ADVISOR_PROMPT_NAME]
    );
  }
}

/**
 * Fetches the Advisor Co-Pilot prompt text from the DB.
 * Returns null if not found.
 */
export async function getAdvisorSystemPrompt(): Promise<string | null> {
  const db = await getDatabase();
  const rows = await db.select<SystemPrompt[]>(
    "SELECT * FROM system_prompts WHERE name = ?",
    [ADVISOR_PROMPT_NAME]
  );
