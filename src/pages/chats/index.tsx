...
function countKYCChanges(messages: ChatMessage[]): number {
  return messages
    .filter((m) => m.role === "assistant")
    .reduce((total, m) => {
      const match = m.content.match(/## KYC Updates \((\d+)\)/);
                ...
                <div key={dateKey} className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {groupedConversations[dateKey].map((doc) => {
                      const kycCount = countKYCChanges(doc.messages);
                      return (
                      <Card
                        key={doc.id}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <p className="line-clamp-1 text-sm font-medium mr-8">
                            {doc.title}
                          </p>
                          <div className="flex items-center gap-1">
                            {kycCount > 0 && (
                              <Badge className="text-xs bg-green-700/20 text-green-400 border-green-700/40 hover:bg-green-700/20">
                                {kycCount} KYC {kycCount === 1 ? "change" : "changes"}
                              </Badge>
                            )}
